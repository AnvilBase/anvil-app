...
struct ChatScreen: View {
  ...
  private var conversation: some View {
    ...
    // chat or Clear all in the drawer — is cleared in the same frame the drawer starts sliding
    // shut (see `ChatModel.newChat`), and a guide fading in on a spring of its own while the page
    // moves on another is a guide that visibly isn't riding the page. Cleared, the guide is
    // simply there, from the first frame, and moves with everything else.
        ...
        LazyVStack(alignment: .leading, spacing: 22) {
          ForEach(chat.messages) { message in
            MessageRow(
              ...
              canResend: chat.canResend(message.id),
              ...
              onResend: { chat.resend(message.id) },
