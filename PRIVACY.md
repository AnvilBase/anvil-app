...
returns Brave's answer; the relay keeps no record of the query, and the app never sees the key. Brave's
...
nothing that identifies you or your phone. The reply lists every query it ran. A build made with a
...
- With a model installed, turn web search off and chat in airplane mode. In **Settings › Privacy & Security › App Privacy Report**, the app should show no
  network activity at all. With web search on, the only domain should be `www.anvilai.com` (or
  `api.search.brave.com` in a build made with its own key).
